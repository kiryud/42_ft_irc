...
- RFC문서 및 irc protocol 분석 : 2024.01.13 ~ 01.16
	- RFC문서(RFC 1459, 2810, 2811, 2812)에 RPL, ERR 메세지 규약이 제대로 없는 경우가 많음
		- 여러가지 RFC 문서 중 해당 부분을 발췌독
		- IRCprotocol의 RPL, ERR메세지를 기반으로 검색해 IRCv3같은것도 찾아냄
	- 확실한 검증을 위해 상용 irc서버와 irssi의 통신을 tcp 통신을 열람해서 확인

[남아있는 분석 기록1](<./subject와 RFC 1459 대강 정리.md>) (이후 팀장님 노션에서 분석함)
...
- 서버와 irc로직을 분리하기로 결정 : 2024.01.25
	- 서버 1명, irc 프로토콜 2명의 구성에서 서로의 개발 편의와 메모리 관리 등을 위하여 서버 객체와 irc 객체를 나누어 작성하기로 함
		...
		- 사용해야하는 정보에 따른 데이터를 담고있는 인스턴스의 lifrcycle을 고려하여 구성함
...
복습하다 기억난건데 파일 전송의 경우 상용 클라이언트끼리 직접 연락 가능하게 다리를 놔주는 형태로 서비스가 제공되며 해당 명령어는 DCC이고 PRIVMSG를 사용한다.

우리 명령어는 당연히 규정대로 "\r\n"을 마지막 문자열 뒤에 붙였지만 평가중 마지막 문자열과 "\r\n" 사이에도 공백을 넣는 팀이 있었는데 DCC가 작동하지 않았다.

RPL MSG의 서식은 당연히 같을테니 문자열을 실제로 어떻게 합쳐 보내는지 확인해봤었고 바로 찾아낼 수 있었다.
